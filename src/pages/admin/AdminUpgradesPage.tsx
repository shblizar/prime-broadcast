  ...
  return (
    <div className="space-y-6" id="admin-upgrades-page">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Kelola opsi upgrade perangkat kamera, wireless video transmitter, monitor switcher, dll.
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-xs text-slate-400">Memuat upgrade...</div>
        ) : upgrades.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3">Nama Upgrade</th>
                  <th className="px-4 py-3">Deskripsi</th>
                  <th className="px-4 py-3">Harga</th>
                  <th className="px-4 py-3">Satuan</th>
                  <th className="px-4 py-3">Batas Kuantitas</th>
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full flex flex-col border border-slate-200">
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <h3 className="font-extrabold text-base text-[#081A2E]">
                {editingId ? 'Edit Upgrade' : 'Tambah Optional Upgrade'}
              </h3>
              <button
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Nama Upgrade *</label>
                <input
                  type="text"
                  required
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Contoh: Tambahan Kamera Cinema FX3"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Spesifikasi atau benefit upgrade"
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#081A2E] outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Label Satuan *</label>
                  <input
                    type="text"
                    required
                    value={unitLabel}
                    onChange={(e) => setUnitLabel(e.target.value)}
                    placeholder="Contoh: /unit, /kamera"
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#081A2E] outline-none"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <label htmlFor="allow-quantity" className="font-bold text-slate-700 cursor-pointer">
                  Izinkan Klien Mengatur Jumlah (Quantity)
                </label>
              </div>
